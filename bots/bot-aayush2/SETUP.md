...
> **STATS_PORT = 3006** (must be unique — nitin=3010, abhi=3004, sachin2=3005, aayush2=3006).
...
> warns at boot; check with `grep 📒 logs/bot-aayush2-out.log`.


## 5. Start + scan
```bash
pm2 start ecosystem.config.cjs --only bot-aayush2     # or: pm2 reload ecosystem.config.cjs
```
...
```bash
pm2 reload ecosystem.config.cjs --only bot-aayush2
```
