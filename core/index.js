...
export async function startBot(config, log, authDir) {
  ...
  async function connectToWhatsApp() {
    ...
    try {
      ...
      commandParser = createCommandParser(store, groupManager, config, log, sock, BOT_START_TIME, trialEngine, removalEngine, ghostEngine, adminLids, reminderSender, getSock, dripEngine, ledger, deliveryTracker);
        ...
        if (connection === 'open') {
          ...
          if (!schedulerStarted) {
            ...
            if (isTracker(config)) {
              ...
              //
              // Addressed to dripIds, not to everyone allowed to type commands. Those are two
              // different lists on purpose: allowedTelegramIds is the security boundary, and a
              // bot the partners can all command is not necessarily a bot they all want the
              // 9 PM summary from. dripIds is who asked to be told, so it is who gets told.
              ...(config.usesTelegram ? {
